...
- Domain、data、presentation のいずれかに機能を追加するとき。
...
4. data 層の詳細を presentation 層へ漏らさず、必要なら mapper や Repository implementation に閉じる。
5. UI を変更する場合は Compose / Material 3、StateFlow、文字列リソース、ライト / ダーク対応を確認する。
6. 変更した振る舞いに対して、`docs/unit-test.md` に沿って UnitTest を追加・修正する。
7. 可能な範囲で lint / UnitTest / build などの検証を実行し、未実行のものは理由を残す。
