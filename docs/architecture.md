...
- `ui`: Compose 画面、ViewModel、UI state
  - `screen`: 状態を持たない `XxxScreen`
  - `route`: ViewModel と画面を接続する `XxxRoute`
  - `viewmodel`: 画面ごとの `XxxViewModel`
  - `state`: UI state（`XxxState` / `XxxUiState`）
  - `model`: 表示用モデル（`XxxUiModel`）
  - `action`: UI callback をまとめる `XxxActions`
  - `event`: 一回限りの UI event（`XxxEvent`）
  - `auth`: 認証など Android Framework と連携する UI ヘルパー（例: `AppLockAuthenticator`）
  - `util`: 画面から呼ぶユーティリティ（例: `launchShareMemo`）
  - `type`: 役割別パッケージに当てはまらない enum（例: `AppLockStatus`、`MonthSwipeDirection`）
  - `data`: 役割別パッケージに当てはまらない表示・受け渡し用 data class（例: `SettingsUiFlags`、`OssLicense`）
  - `component`: 再利用する Compose 部品
  - `navigation`: 画面遷移先の定義（`LiteMemoDestination`）と NavHost 配線
  - `theme`: Compose / Material 3 テーマ
  - `widget`: Glance ウィジェット。機能内では `common` / `data` / `di` とウィジェット別パッケージに分けてよい
...
- Room はメモ・タグ・メモ画像メタデータなどの構造化データを扱う。スキーマは `app/schemas/` にエクスポートし、変更時は migration を追加する
...
- メモ一覧・検索・カレンダー表示では先頭画像だけをサムネイルとして扱う
- 添付画像を削除するときは、未保存画像の cleanup と保存済み画像の repository 差分削除を分けて扱う
